...
_DISK_READ_QUERY = (
    'rate(container_fs_reads_bytes_total{{namespace="{namespace}", pod="{pod}"}}[5m])'
)
_DISK_WRITE_QUERY = (
    'rate(container_fs_writes_bytes_total{{namespace="{namespace}", pod="{pod}"}}[5m])'
)
_NET_RX_QUERY = (
    'rate(container_network_receive_bytes_total{{namespace="{namespace}", pod="{pod}"}}[5m])'
)
_NET_TX_QUERY = (
    'rate(container_network_transmit_bytes_total{{namespace="{namespace}", pod="{pod}"}}[5m])'
)
...
def _safe_timestamp(response: dict[str, Any]) -> float | None:
    """Safely extract timestamp from Prometheus response."""
    try:
        ts = response.get("data", {}).get("result", [{}])[0].get("value", [None])[0]
...
async def get_cpu_usage(
    ...
    if range:
        ...
        return [{"timestamp": ts, "value": float(val)} for ts, val in values]
...
async def get_disk_io(namespace: str, pod: str) -> dict[str, MetricResult]:
    ...
    return {
        "read_bytes_sec": MetricResult(
            ...
            query=read_query,
            timestamp=_safe_timestamp(read_resp),
        ),
        "write_bytes_sec": MetricResult(
            value=_extract_value(write_resp),
            query=write_query,
            timestamp=_safe_timestamp(write_resp),
        ),
    }
...
async def get_network_io(namespace: str, pod: str) -> dict[str, MetricResult]:
    ...
    return {
        "receive_bytes_sec": MetricResult(
            ...
            query=rx_query,
            timestamp=_safe_timestamp(rx_resp),
        ),
        "transmit_bytes_sec": MetricResult(
            value=_extract_value(tx_resp),
            query=tx_query,
            timestamp=_safe_timestamp(tx_resp),
        ),
    }
